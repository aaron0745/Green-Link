...
import { Calendar as CalendarIcon, Home, Users, User, Clock, IndianRupee, MapPin, Loader2, CheckCircle2, CreditCard, QrCode, ClipboardList, RefreshCw, MoreHorizontal, Filter } from "lucide-react";
...
export default function Dashboard() {
  ...
  const stats = (() => {
    ...
    // Deduplicate logs by householdId to get accurate household counts
    const uniqueLogsMap = new Map();
    filteredLogs.forEach((l: any) => {
      const existing = uniqueLogsMap.get(l.householdId);
      // Prioritize 'paid' status over 'collected' if both exist for the same house
  ...
  const displayLogs = (() => {
    ...
    const logsForDay = collectionLogs.filter((log: any) => {
      ...
      const ds = formatDateForQuery(selectedDate);
      const matchesDate = log.timestamp.includes(ds) || log.timestamp.includes(selectedDate.toLocaleDateString('en-GB'));
    ...
    logsForDay.forEach((l: any) => {
      const existing = displayMap.get(l.householdId);
      if (!existing || l.status === 'paid' || (existing.status !== 'paid' && l.status === 'collected')) {
        displayMap.set(l.householdId, l);
      }
    });
  ...
  const chartData = (() => {
    ...
    const totalHouses = households?.length || 0;
      ...
      <div className="space-y-6 max-w-[1600px] mx-auto">
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-4 md:space-y-6 mt-0">
            {/* Widget Grid - Forced to stay within bounds */}
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
              {widgets.map((w) => (
                <Card key={w.label} className="border-border/60 shadow-sm hover:shadow-md transition-shadow border-none md:border-solid bg-card/50 md:bg-card">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
               {/* Main Chart Area */}
               <Card className="col-span-1 lg:col-span-2 border-border/60 shadow-sm border-none md:border-solid overflow-hidden">
               ...
               {/* Live Map */}
               <Card className="col-span-1 border-border/60 shadow-sm flex flex-col overflow-hidden border-none md:border-solid">
